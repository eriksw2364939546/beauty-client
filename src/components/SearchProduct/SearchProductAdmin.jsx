...
const SearchProductAdmin = ({ initialSearch = "" }) => {
  ...
  const handleSearchDebounced = useCallback(() => {
    ...
    // Обработка поиска для админки (тоже минимум 2 символа)
    ...
    // В админке сбрасываем пагинацию не нужно, если она есть
    // params.delete("page");

    const queryString = params.toString();
    // Обновляем URL для админской части
    router.push(
      queryString
        ? `/beauty-admin/products-admin?${queryString}`
        : "/beauty-admin/products-admin",
      {
        scroll: false,
      }
    );
